...
- [`tests/designer/features/ide-vim.test.ts`](../../../../tests/designer/features/ide-vim.test.ts): Vim mode (no load with the setting off, on at open, live toggle, the mode line, Esc in each mode, the ex commands, the clipboard sync, read-only sources, Ctrl+S / Shift+Alt+F / Ctrl+F, keys not reaching the page).
