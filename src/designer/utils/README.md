...
- **Used by:** `TitleUpdater` (method and page names), `KeyboardShortcuts` (method name and category for the Shift+L link), `OutputCopy` and `TextareaEditor` (`textareaUnderPointer`, `copyText`), the JSON editor (`firstMatch`), and the editor modal (`copyText`).
