...
Designer features (`src/designer/features/`): `title-updater` (tab title), `keyboard` (shortcuts; `ad-link.ts` is Shift+L's link), `run-test` (Run Test lookup + click, `runTestAction`), `json-editor` (JSON modal: extractor, sync engine, type adapters), `output-copy` (hover copy icon on AD's outputs; AD only), `ide` (shared hover overlay + lazy CodeMirror modal, with `#{variable}` completion/hover/lint, a lazily loaded SQL/JSON formatter and an optional, lazily loaded Vim mode), `ad-scope` (AD-only scanner of the `#{variables}` in scope, injected into `ide`), `curl-autofill` (autofill from the AD Network panel's handoff; started directly by `ad-content.ts`), `settings` (the always-on ⚙ button, shortcuts and Settings modal).
...
- **Timings.** Waits tuned against the designers' rendering (widget polls, pauses after clicks, first-try delays, `rearmDelays`, `pdRoutePoll`, `jsonButtonThrottle`) are in `src/config/timings.ts`, together with numbers two worlds must agree on (`panelFocusFlash`, which `FocusFlash` in `devtools/view.ts` also hands to the panels' CSS animation as `--focus-flash-ms`; `pdInspectHeartbeat` / `pdInspectTimeout`) and the AD Network lookup retry schedule (`resolveRetry`). Timings of the extension's own UI (hover grace, Esc Esc window, the IDE's discard window in `ide/footer.ts`) stay next to their code.
- **Names.** Everything the extension adds to a shared world carries the one `belz` prefix from `config/namespace.ts`: DOM ids, classes and data attributes (`ns()`, `nsAttr()`), globals in the inspected page (`PAGE_GLOBALS`), message keys (`COMMAND_MESSAGE_KEY`, `HOSTS_MESSAGE_KEY`, `AUTOFILL_MESSAGE_KEY`) and the autofill fragment marker. The storage keys in `config/storage-keys.ts` keep their `sdExtension…V1` names, whose `V1` is the version of the shape stored under them: before the first release keys and shapes may change freely; from the first release on, an incompatible change needs a new key or a migration (`config/README.md`, "Stored shapes").
...
In `ide/modal.ts`, one `keydown` listener on `window`, capture phase, handles `Esc`, `Ctrl/Cmd+S`, `Ctrl/Cmd+F` and `Shift+Alt+F` (Format), and only while the IDE is the topmost modal. `Shift+Alt+F` is matched on the physical key only (`event.code === 'KeyF'`), because Option changes the character on a Mac; `Ctrl/Cmd+S` and `Ctrl/Cmd+F` are matched on the character (`event.key`), like the browser's Save page and Find they stand in for. It runs before every listener on the document and below, and before window-capture listeners added after the IDE first opened; a key it handles gets `preventDefault()` and `stopImmediatePropagation()`, so neither the page's shortcuts nor the browser's Save page / Find act on it. `Esc` steps aside while CodeMirror has its completion list or search panel open (`completionStatus()`, `searchPanelOpen()`), so CodeMirror's own `Esc` closes those first. `Esc` and a click on the backdrop close alike (`closeOrAskFirst()`): with unsaved changes, the first shows a prompt in the footer and a second (either) within the discard window (3 s) discards; an edit in between withdraws the prompt. On a read-only source a Format alone is not an unsaved change; any other change is. The footer line has one owner, `FooterStatus` (`ide/footer.ts`): the prompt while armed, else a Format message within its time, else the open's own text; a Format message never replaces an armed prompt, and the prompt ends by itself when its window closes. **Cancel** and **×** close without asking. No `window.confirm`. Keys typed in the IDE that the editor leaves alone stop at the IDE overlay (a bubble-phase `stopPropagation` for `keydown`, `keypress`, `keyup`), so they reach none of the page's bubble listeners; the extension's own page shortcuts already do nothing while a modal holds `modalLock`.

With **IDE Vim Mode** on, Vim goes first: see "IDE Vim mode" below.

Every other key, including `Ctrl+Backspace` / Mac `Alt+Backspace` (CodeMirror's `deleteGroupBackward`), passes the window listener untouched. `Ctrl+W` is the browser's (close tab) and reaches no page or extension, so the IDE guards unsaved work instead: `syncUnloadGuard()` puts one `beforeunload` listener (`onBeforeUnload`: `preventDefault()` + `returnValue = ''`, the browser's "Leave site?" dialog) on `window` while `hasUnsavedChanges` is true and takes it off when it turns false. It runs on every text change, after `:w`, on open and on close, and touches `window` only when the state flips; `dispose()` removes it too. It never stops propagation, so the page's own `beforeunload` listeners run as before. The content script's `window` is the page's, and a content-script `beforeunload` listener works in Chrome and Firefox.

## IDE Vim mode

`ideVim` (ide section, default off). `ide/vim.ts` with `@replit/codemirror-vim` (exact-pinned 6.4.0; its engine `@replit/codemirror-vim-core` comes with it) is its own chunk (~125 KB), requested only while the setting is on: `Ide.openIdeFor()` starts `import('./vim')` alongside `import('./modal')` when an open begins with the setting on, and `IdeModal.syncVim()` (after every `createView()` and every settings change) puts `vimExtension(host)` into a `Compartment` that is **first** in the view's extensions (the library's keys before every other keymap), or empties it, so a toggle applies live. Off, nothing requests the chunk. It holds state (the library's one global Vim: registers, ex commands) and carries the `designer/features/ide/vim` singleton marker. The CodeMirror modules it shares with the IDE move into a chunk both load; the extra CodeMirror code only Vim uses (~18 KB) is therefore in the IDE's closure too, because the bundler splits per module.

- **Esc** with Vim on (`escapeBelongsToVim()`): an open completion list or hover tooltip is closed by the IDE (and the key consumed, so Vim does not also leave insert mode); with the search panel or Vim's `:` / `/` prompt open, or Vim in insert, replace or visual mode, or keys pending (count, register, operator, partial command; read from `getCM(view).state.vim` by `vimKeyState()`), the IDE steps aside and Vim acts. Only normal mode with nothing pending reaches `closeOrAskFirst()`.
- **Ex commands** (`Vim.defineEx`, once per page, dispatched to the view's `VimHost` through a `WeakMap`, after the library's key handling has finished): `:w` writes the text back and stays open (`writeAndStay()`: the written text becomes the new baseline for unsaved changes); `:q` is `closeOrAskFirst()`; `:q!` closes; `:wq` is Save; `:x` saves when there are unsaved changes, else closes. On a read-only source the writes show the same toast as Save and nothing closes.
- **Clipboard**, like Vim's `clipboard=unnamedplus`: the library has no hook (`defineRegister()` refuses the existing unnamed register), so `syncClipboard()` wraps `pushText()` on the instance the public `Vim.getRegisterController()` returns, once per controller. After each yank, delete or change that names no register (and `:yank`, which names register 0), the unnamed register's text goes to `IdeModal.copyToClipboard()` → `copyText()` (Clipboard API, `execCommand('copy')` fallback, then focus back to the editor), in a microtask after the key. `"_`, named registers and `"+` (the library writes `"+` to the clipboard itself) are not copied; a visual `p` sets the unnamed register directly and is not copied either. `p` reads Vim's register only; `"+p` is the library's `navigator.clipboard.readText()` (no `clipboardRead` permission is declared, so the browser may prompt).
- **IDE keys:** `Ctrl/Cmd+S` and `Shift+Alt+F` stay the IDE's in every mode; `Ctrl/Cmd+F` is not handled while Vim is on (Vim's page down; search is `/`). Every other key reaches CodeMirror and Vim. In insert mode the library handles `Ctrl+U` (delete to line start) and `Ctrl+W` (which the browser never delivers: it closes the tab); `Ctrl+Backspace` falls through to CodeMirror's `deleteGroupBackward`; `Ctrl+H` is `insertModeKeys` in `vim.ts` (`deleteCharBackward`, insert mode only; the library leaves it alone and CodeMirror binds it only on a Mac, so the browser's History would take it).
- **Footer:** `VimBridge` (a view plugin after `vim()`) follows the library's `vim-mode-change`, `vim-keypress` and `vim-command-done` events and passes `modeLine()` (`-- NORMAL --`, `-- VISUAL LINE --  2d`, …) to `FooterStatus.setMode()`, which puts it before the line's other text; the discard prompt still shows alone while armed.
- **Look:** `drawSelection()` is part of the extension (Vim mode hides the browser's selection, so visual selections are drawn); the block cursor is the accent blue instead of the library's pink.
- **Read-only sources:** the library refuses changes and insert mode on `EditorState.readOnly`; moves, search and yanks work.

## IDE Format

`IdeModal.format()` (the header's **Format** button and `Shift+Alt+F`) formats SQL and JSON only (`isFormattable()` in `ide/language.ts`); in other modes it only says so in the footer. The formatter, `ide/format.ts` with `sql-formatter` (an exact-pinned dependency; `formatDialect` + the `postgresql` dialect only), is its own chunk, loaded by `import('./format')` on the first Format. The result is one CodeMirror transaction (`userEvent: 'format'`, one undo) over the selection or the whole text, none when nothing changes; a selection's later lines get the indentation of the line it starts on; errors leave the text alone, name document lines, and show in the footer. AD syntax is protected, never guessed at: in SQL each `#{ … }` becomes a unique identifier token during formatting and every token must come back exactly once, `:name` / `$1` are declared parameters, keywords are upper case, data types lower case (`::text[]` too) and names as written, and text whose first word does not start a SQL statement is refused ("Doesn't look like SQL"); JSON is validated with `JSON.parse` (bare placeholders standing in as `""`, and, when that fails, placeholders inside strings taken whole, so one holding double quotes formats) and re-laid out from its own tokens, so numbers and escapes are copied, not re-serialised. A read-only source can be formatted for reading; a Format there never counts as unsaved. Details in [`src/designer/features/ide/README.md`](src/designer/features/ide/README.md).
...
The IDE modal (`designer/features/ide/modal.ts`) carries CodeMirror and every language mode, ~630 KB (with `@codemirror/lint` for the variable linter), in its own chunk plus a CodeMirror chunk it shares with Vim mode. It is reached only through `import('./modal')` in `ide/index.ts`, and fetched on the first **Open** click (then cached; a reopen takes ~50 ms). A page load parses about 53 KB on AD pages and 50 KB on PD pages; `tests/build/bundle.test.ts` keeps it under 100 KB.
...
The IDE chunk itself lazy-loads one more chunk, the formatter (`ide/format.ts` + `sql-formatter`, ~75 KB), on the first Format; it shares the small `references.ts` chunk with the IDE and holds no state. Vim mode (`ide/vim.ts` + `@replit/codemirror-vim`, ~125 KB) is another, loaded only with the IDE Vim Mode setting on (see "IDE Vim mode"); it shares the CodeMirror chunk with the IDE, so CodeMirror is still loaded once.
