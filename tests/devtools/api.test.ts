...
import { MethodResolver } from '../../src/devtools/ad-network/api';

const site = new InspectedSite();
const cache = new MethodCache();
const resolver = new MethodResolver(site, cache);
...
// Each test uses its own uuid: the resolver shares one in-memory cache.
let n = 0;
...
describe('resolveSummary', () => {
  ...
  test('a 401 is reported as "not signed in" without trying v1', async () => {
    ...
    await expect(resolver.resolveSummary(nextUuid())).rejects.toThrow('not signed in');
