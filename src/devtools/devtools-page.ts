...
// only registers the panels; all logic lives in the panel pages (ad-network/, pd-inspector/), which
...
/** Adds the panels to this DevTools window, once, when it inspects an allowed site. */
class PanelRegistrar {
  ...
  start(): void {
    ...
    // DevTools.
    chrome.devtools.network.onNavigated.addListener(() => this.tryCreate());
    // Same for a host added to the allowed list via the options page while
    // DevTools is already open.
    chrome.storage.onChanged.addListener((changes, areaName) => {
      if (isHostsChange(changes, areaName)) this.tryCreate();
    });
  }
