...
/** One key/value row of the tree, with its DOM and its children. */
interface JsonNode {
  /** Property name or array index; null for a bare root value. */
  key: string | null;
  value: unknown;
  /** An object or array (has children to expand). */
  container: boolean;
  childNodes: JsonNode[];
  expanded: boolean;
  el: HTMLDivElement;
  row: HTMLDivElement;
  childrenEl: HTMLDivElement | null;
}
...
function previewOf(value: object): string {
  ...
  const shown = keys
    .slice(0, 3)
    .map((k) => k + ': ' + previewToken((value as Record<string, unknown>)[k]));
...
function makeNode(key: string | null, value: unknown, depth: number): JsonNode {
...
function setExpanded(node: JsonNode, expanded: boolean): void {
...
function eachContainer(nodes: JsonNode[], fn: (node: JsonNode) => void): void {
...
function applyFilter(nodes: JsonNode[], query: string): void {
  const q = query.trim().toLowerCase();

  const visit = (node: JsonNode, ancestorMatched: boolean): boolean => {
    const keyMatch =
...
export function createJsonView(rawText: unknown): { element: HTMLElement } {
