...
//
// Read by four worlds — background (script registration, grant sync; the
// only writer), options page (the editor, which asks the background to make
// each change), DevTools page (panel gating) and the AD Network panel
// (allowed-site check, designer host overrides) — so the storage shape, its
// validation and the allowed-site check live here once.
//
...
/**
 * Store the list. Only the background writes it (ContentScriptSync, which
 * makes every change one at a time); tests use it to set storage up.
 */
...
export async function isGranted(host: string): Promise<boolean> {
  try {
...
/**
 * Each of `hosts`' grant as the browser reports it now: the browser, not the
 * stored `enabled` flag, is the authority. Reads only.
 */
export async function readGrants(hosts: readonly HostEntry[]): Promise<Map<string, boolean>> {
  const grants = new Map<string, boolean>();
  for (const entry of hosts) grants.set(entry.host, await isGranted(entry.host));
  return grants;
}
