...
import { startPendingCapture } from './pending-capture';
import {
  detectOrigin,
  getApiHost,
  loadSiteConfig,
...
// ---- DOM ------------------------------------------------------------------
const recordBtn = required<HTMLButtonElement>('#record');
const clearBtn = required<HTMLButtonElement>('#clear');
const preserveBox = required<HTMLInputElement>('#preserve');
const filterInput = required<HTMLInputElement>('#filter');
const countEl = required('#count');
const offlineEl = required('#offline');
const listPane = required('.list-pane');
const rowsEl = required<HTMLTableSectionElement>('#rows');
...
const detailCopy = required<HTMLButtonElement>('#detail-copy');
const detailTabs = Array.from(document.querySelectorAll<HTMLButtonElement>('.detail-tabs button'));
const toastEl = required('#toast');
const pendingRowsEl = required<HTMLTableSectionElement>('#pending-rows');

// ---- state ----------------------------------------------------------------
const entries: Row[] = []; // chronological (oldest-first by start time), mirrors DOM order
const seen = new Set<string>(); // dedup key = request.url + '|' + start time in epoch ms
const uuidToName = new Map<string, string>();
const uuidToCategory = new Map<string, string>();
let nextId = 1;
let recording = true;
let preserveLog = false;
let filterText = '';
let selectedId: number | null = null;
let activeTab: DetailTab = 'headers';
let currentCopyText = '';
...
/** In-flight rows from pending-capture.ts: pending id -> tr element. */
const pendingRows = new Map<number, HTMLTableRowElement>();

// "Open in draft" queue — processed one at a time.
const openQueue: Row[] = [];
let openProcessing = false;

// Origin detection + designer-host mapping live in ./origin.ts.
...
function typeOf(har: HarEntry): string {
...
function transferSize(har: HarEntry): number {
...
function statusGroup(status: number | null | undefined): StatusGroup {
...
function iconButton(svg: string, title: string, handler: (button: HTMLButtonElement) => void): HTMLButtonElement {
...
// takes).
async function copySlackLink(entry: Row, btn: HTMLButtonElement): Promise<void> {
  try {
    await ready;
    ...
    setOffline(true, errorText(err));
    showToast(
      'could not copy link — ' +
        (errorText(err) || 'lookup failed')
    );
  }
}
...
function enqueueOpen(entry: Row): void {
...
async function processOpenQueue(): Promise<void> {
...
function learnName(uuid: string, name: string): void {
...
function learnCategory(uuid: string, category: string): void {
...
function paintCategory(entry: Row): void {
  const cat = uuidToCategory.get(entry.uuid);
  if (!entry.categoryCell) return;
...
function setOffline(off: boolean, reason?: string): void {
...
function applySummary(uuid: string, summary: MethodSummary | null): void {
  if (!summary) return;
  if (summary.name) learnName(uuid, summary.name);
  if (summary.category) learnCategory(uuid, summary.category);
}

function needsResolve(uuid: string): boolean {
...
async function flushResolve(): Promise<void> {
  ...
  setOffline(failed.length > 0, errorText(lastError));
...
function harKey(har: HarEntry): string {
...
function startedAt(har: HarEntry): number {
...
function insertIndexFor(started: number): number {
...
function renumberRows(): void {
  for (let i = 0; i < entries.length; i++) {
    const cell = entries[i]!.rowEl?.firstChild;
    if (cell) cell.textContent = String(i + 1);
  }
}

function onRequest(har: HarEntry): void {
  if (!recording) return;
...
function renderRow(entry: Row, insertAt?: number): void {
...
// ---- filter ---------------------------------------------------------------
function rowMatches(entry: Row): boolean {
...
function applyRowFilter(entry: Row): void {
...
// ---- detail pane ----------------------------------------------------------
function selectEntry(entry: Row): void {
  selectedId = entry.id;
...
function kvGrid(pairs: Array<[string, unknown]>): HTMLElement {
...
function headerRows(headers: Array<{ name: string; value: string }> | undefined): Array<[string, unknown]> {
  return Array.isArray(headers) ? headers.map((h): [string, unknown] => [h.name, h.value]) : [];
}

function headersToObj(headers: Array<{ name: string; value: string }> | undefined): Record<string, string> {
...
function prettyMaybeJson(text: unknown): string {
...
function showResponseBody(body: string): void {
...
function showResponseUnavailable(entry: Row): void {
  currentCopyText = '';
  const note =
    entry.statusGroup === 'error'
...
function renderResponseTab(entry: Row, har: HarEntry): void {
...
recordBtn.addEventListener('click', () => {
  ...
  const label = recordBtn.querySelector('.dot')?.nextSibling;
  if (label) label.textContent = recording ? ' Recording' : ' Paused';
});
...
for (const tab of detailTabs) {
  tab.addEventListener('click', () => {
    activeTab = (tab.dataset.tab as DetailTab | undefined) ?? 'headers';
...
function updatePending(entries: PendingEntry[]): void {
...
watchFocusFlag('ad', () => {
  ...
  if (last && last.rowEl) {
    const row = last.rowEl;
    row.classList.add('focus-flash');
    setTimeout(() => row.classList.remove('focus-flash'), 900);
  }
  filterInput.focus();
});
